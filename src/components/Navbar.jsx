...
const Navbar = ({
    ...
    const renderCycleList = (mode, index) => {
        return (
            <li
                key={index}
                className='pl-2 py-2 cursor-pointer bg-gray-200-hover flex flex-shrink-0 items-center'
                style={{ paddingRight: '13.13px' }}
                onClick={() => mode === 0 ? selectCycle(mode + 1) : selectCycle(mode)}
            >
                <span className='block w-5 h-5 mr-2 text-blue-700-g'>
                    <Icon style={{ width: '100%', height: '100%' }} icon={['far', 'calendar-alt']} />
                </span>
                <span className='mr-2 text-lg text-blue-700-g flex-shrink-0'>
                    {calendarType[mode]}
    ...
    return (
        <div className='flex justify-between bg-gray-100-g select-none' ref={outerRef}>
            <ul className='flex py-2'>
                <li
                    className='p-2 mx-1 cursor-pointer bg-gray-200-hover flex text-lg flex-shrink-0 text-blue-700-g'
                    onClick={() => setMonth(setShowData, showData)}
                >
                    <span className='flex items-center mr-2 text-blue-700-g'>
                        <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-blue-700-g" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <li
                    className='p-2 px-4 mx-1 cursor-pointer bg-gray-200-hover flex-shrink-0'
                    onClick={() => handleSetDate(-1)}
                >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-blue-700-g" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                    </svg>
                </li>
                <li
                    className='p-2 px-4 mx-1 cursor-pointer bg-gray-200-hover flex-shrink-0 relative'
                    onClick={() => handleSetDate(1)}
                >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-blue-700-g" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <li
                    className={`p-2 mx-1 cursor-pointer text-gray-300-g bg-gray-200-hover flex-shrink-0 rounded-sm ${!showCalendar ? '' : 'bg-blue-100-g'}`}
                    onClick={() => setShowCalendar(!showCalendar)}
                    ref={selectMonthRef}
                >
                    <span className='mr-2 text-blue-700-g'>
            ...
            <ul className='flex py-2'>
                <li
                    className='p-2 mx-1 cursor-pointer flex items-center text-gray-300-g bg-gray-200-hover flex-shrink-0'
                    onClick={() => setShowCycle(!showCycle)}
                    ref={selectCycleRef}
                >
                    <span className='w-5 h-5 mr-2 text-blue-700-g'>
                        <Icon icon={['far', 'calendar-alt']} />
                    </span>
                    <span className='mr-2 text-lg text-blue-700-g'>
                        {calendarType[cycle]}
            ...
            {!showCalendar ? null : createPortal(
                <div className={`fixed h-full w-full top-0`}>
                    ...
                    <div
                        className={`absolute bg-white rounded-md shadow-lg-black ring-1-op10-black transition-g ease-out-g duration-75-g flex`}
                    ...
                    >
                        {cycle === 30 ? null : <div className='flex flex-col p-2 pb-5' style={{ width: '219px', height: '215.7px' }}>
                            <div className='flex justify-between select-none'>
                                ...
                                <ul className='flex'>
                                    <li
                                        className='p-1 cursor-pointer bg-gray-200-hover flex-shrink-0'
                                    ...
                                    <li
                                        className='p-1 cursor-pointer bg-gray-200-hover flex-shrink-0 relative'
                                        onClick={() => setMonth(setPreviewData, previewData, 1)}
                                    >
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
